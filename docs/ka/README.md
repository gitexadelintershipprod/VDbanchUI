...
| `master_id_rsa.pub` | **ყველა slave** (master-დან კოპირება) | Master-ის საჯარო გასაღები — **აუცილებელია `02` / `03`-მდე** |
...
> | `C:\install\master_id_rsa.pub` | **Slave-ებისთვის ექსპორტი** (იგივე საჯარო გასაღები — ეს დააკოპირე) |
>
> **დააკოპირე `master_id_rsa.pub` ყველა slave-ზე** მათ install საქაღალდეში:
>
...
- წერს `C:\install\master_id_rsa.pub`-ს slave-ებზე გასავრცელებლად  
...
მხოლოდ მას შემდეგ, რაც `master_id_rsa.pub` უკვე არის slave-ის `C:\install`-ში:
...
- `OpenSSH-Win64-*.msi`  
- `vdbench*.zip`  
- **`master_id_rsa.pub`** (master-დან — იხ. §3.2)  
...
განათავსე `/root/install`-ში (სკრიპტი + `vdbench*.zip` + **`master_id_rsa.pub`** + RPM-ები / `rpms/`), შემდეგ:
...
1. **Run**-ზე გამოიყენე **Config only** — მხოლოდ `.parm` და საქაღალდე, Vdbench გარეშე.  
2. შეამოწმე, რომ გაშვების საქაღალდე გამოჩნდა Reports root-ში.  
3. მოკლე რეალური გაშვებისთვის გამოიყენე ცარიელი filesystem ანქორი (არა პროდაქშენი).  
4. **Start**-მდე ნახე **Preview**-ის `RISK` გაფრთხილებები.  
